...
while True:
    ...
    blur = cv2.blur(noised_img,(5,5)) # (10, 10) => 더 뿌얘지게 만들어 준다.     
    ...
    # cv2.imshow("median", median_filtered) # 메디안 필터를 적용한 영상 출력 salt & pepper noise에 효과적
    
    # 여러 영상을 하나로 합칠때 사용
    total = cv2.hconcat([cv2.vconcat([frame, noised_img]), cv2.vconcat([blur, gaussian]), cv2.vconcat([bilateral, median_filtered])])
    # vconcat : 세로로 영상을 쌓는 것
    # hconcat : 가로로 영상을 붙여주는 것
...
# 영상 데이터의 구조
# numpy array 형태
# (height, width, 3) 차원 형태 - 색상은 R, G, B 순서이며, 픽셀값은 0 ~ 255까지 256단계
# (127, 259, 0)의 값을 128로 바꾸면 => height = 127, width = 259, 파란색의 밝기가 128이 된다.
