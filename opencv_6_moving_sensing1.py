...
def get_diff_img(frame_a, frame_b, frame_c, threshold):
    ...
    # value : 적용되는 flag에 따라 픽셀 문턱값보다 적을 때 적용되는 최대값
    # flag : 문턱값 적용 방법 또는 스타일
    
    # cv2.THRESH_BINARY : 픽셀값이 threshold_value보다 크면 value, 작으면 0으로 할당 = threshold_value 보다 밝으면 하얀색(value = 255), 어두우면 검정색(0)
    # cv2.THRESH_BINARY_INV : 픽셀값이 threshold_value 보다 크면 0, 작으면 value로 할당 = threshold_value 보다 밝으면 검정색(0), 어두우면 하얀색(value = 255)
    # cv2.THRESH_TRUNC : 픽셀값이  threshold_value보다 크면 threshold_value, 작으면 픽셀값 그대로 할당 = threshold_value 보다 밝으면 threshold_value, 작으면 그대로 
    # cv2.THRESH_TOZERO : 픽셀값이 threshold_value 보다 크면 픽셀 값 그대로, 작으면 0으로 할당 = threshold_value 보다 밝으면 그대로 어두우면 검정색
    # cv2.THRESH_TOZERO_INV : 픽셀값이 threshold_value 보다 크면 0, 작으면 픽셀값 그대로 할당 = threshold_value 보다 밝으면 검정색 어두우면 그대로
...
capture = cv2.VideoCapture(0) # 캠이 하나일경우 0으로 지정해야한다. 그렇지 않으면 화면이 안나옴.
...
# threshold, diff_max의 값이 작을 수록 민감해진다.
