...
while True:
    ...
    nowDatetime_path = now.strftime('%Y-%m-%d %H_%M_%S') # 파일이름으로는 :를 못쓰기 때문에 따로 만들어줌 - 에러가 뜨지 않기때문에 오동작시 원인을 찾기 매우 힘들다!
    ...
    if key == ord('r') and is_record == False:  # 현재 녹화상태가 아니며 r 키를 누르면
        ...
        # 비디오 객체에 (파일이름(한글가능), 인코더, 초당프레임률(정확하지 않음), 영상크기(가로크기, 세로크기)) 로 영상을 쓸 준비
        ...
        cv2.imwrite("capture " + nowDatetime_path + ".png", frame)  # 파일이름(한글안됨), 이미지 - 한글이름 저장시 에러 없는 오동작으로 원인을 찾기 어렵다.
    ...
    if is_record == True:       # 현재 녹화상태이면            
        ...
        cv2.circle(img=frame, center=(625, 15), radius=5, color=(0,0,255), thickness=-1)
    cv2.imshow("output "+str(frame.shape[1]) + "*" + str(frame.shape[0]), frame)   # 현재 시간을 표시하는 글자를 써준 영상 출력
